...
async function recordBillProfit(opts: {
  ...
  /** Actual VTpass total_amount when present — becomes provider_cost */
  vtpassTotalAmount: number | null;
  /** Actual VTpass commission when present */
...
export const requeryBill = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<BillPurchaseResult> => {
    ...
    if (status === "successful" && ["data", "cable", "electricity"].includes(slug)) {
      const providerAmountRaw = Number(meta["provider_amount"]);
      const providerAmount = Number.isFinite(providerAmountRaw) ? providerAmountRaw : Number(bill.amount);
      const rockpayFeeRaw = meta["rockpay_fee"];
      const rockpayFee =
        typeof rockpayFeeRaw === "number" && Number.isFinite(rockpayFeeRaw)
          ? rockpayFeeRaw
          : typeof rockpayFeeRaw === "string" && Number.isFinite(Number(rockpayFeeRaw))
            ? Number(rockpayFeeRaw)
            : null;
      const pricingRuleId =
        typeof meta["pricing_rule_id"] === "string" ? meta["pricing_rule_id"] : null;
      await recordBillProfit({
        reference: bill.internal_reference,
        service: slug as "data" | "cable" | "electricity",
        provider: String(bill.provider ?? ""),
        productCode: String(meta["variation_code"] ?? meta["meter_type"] ?? "") || null,
        customerAmount: Number(bill.amount),
        providerAmount: Number.isFinite(providerAmount) ? providerAmount : Number(bill.amount),
        rockpayFee,
        pricingRuleId,
        vtpassTotalAmount: pay.totalAmount,
        vtpassCommission: pay.commission,
