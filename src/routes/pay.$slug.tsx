...
function PayFlow() {
  ...
  const total = isCable && variation
    ? variation.amount
    : pack
      ? pack.price
      : Number(amount || 0);

  // Load live catalogue for cable / electricity
  useEffect(() => {
    if (!isProviderBill) return;
    let cancelled = false;
    (async () => {
      setCatalogLoading(true);
      setCatalogError("");
      try {
        const category = isCable ? "tv-subscription" : "electricity-bill";
  ...
  }, [isProviderBill, isCable, loadServices]);

  // Load packages when cable provider selected
  useEffect(() => {
    if (!isCable || !serviceID) return;
    let cancelled = false;
    (async () => {
  ...
  }, [isCable, serviceID, loadVariations]);
      ...
      <AppShell>
        ...
        <div className="space-y-3 px-4 pt-1 pb-6">
          ...
          <div className="divide-y rounded-xl border border-border/70 bg-card px-3 py-1">
            ...
            {isElectricity ? <InfoRow label="Meter type" value={meterType} /> : null}
            {variation ? <InfoRow label="Package" value={variation.name} /> : null}
  ...
  if (step === "amount") {
    return (
      <AppShell>
        <PageHeader
          title={isCable || service.mode === "package" ? "Select package" : "Enter amount"}
          subtitle={`${provider || serviceID} · ${maskTail(identifier) || service.name}`}
          onBack={() => setStep(service.verifies || isProviderBill ? "verify" : "identifier")}
        />
        <div className="space-y-3 px-4 pt-1 pb-6">
          <PrefillBanner />
          {isCable ? (
          ...
          <Button
            className="h-11 w-full rounded-xl text-sm font-bold"
            disabled={total < 50 || (isCable && !variation) || (service.mode === "package" && !isCable && !pack)}
            onClick={() => {
              if (isCable && !variation) {
              ...
              if (service.mode === "package" && !isCable && !pack) {
                toast.error("Select a package");
                return;
              }
              setStep("confirm");
        ...
        <div className="space-y-3 px-4 pt-1 pb-6">
          ...
          {verifying ? (
            <div className="flex items-center justify-center gap-2 py-6 text-xs font-semibold text-muted-foreground">
              <Loader2 className="size-4 animate-spin" /> Verifying with provider…
            </div>
          ) : (
            <>
  ...
  if (step === "meterType" && isElectricity) {
    return (
      <AppShell>
        ...
        <div className="space-y-1.5 px-4 pt-1 pb-6">
  ...
  // Provider selection
  const providerOptions = isProviderBill
    ...
    <AppShell>
      ...
      <div className="space-y-3 px-4 pt-1 pb-6">
        ...
        <div>
          ...
          {isProviderBill && catalogLoading ? (
            <div className="flex items-center justify-center gap-2 py-8 text-xs text-muted-foreground">
              <Loader2 className="size-4 animate-spin" /> Loading providers…
            </div>
          ) : isProviderBill && catalogError ? (
