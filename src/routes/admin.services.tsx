...
function AdminServices() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [{ data: breakdown, error: breakdownError }, { data: settings, error: settingsError }] =
        await Promise.all([
          ...
          (
            ...
            .select("service_slug, is_enabled, updated_at")
            .order("service_slug"),
        ]);
      if (breakdownError) setError(breakdownError.message);
      else setRows(Array.isArray(breakdown) ? (breakdown as ServiceRow[]) : []);
      if (settingsError) setError((current) => current ?? settingsError.message);
      else
        setAvailability(Array.isArray(settings) ? (settings as unknown as AvailabilityRow[]) : []);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const toggle = async (slug: string, enabled: boolean) => {
    setSaving(slug);
    setError(null);
    const { error: err } = await asLooseRpc(supabase.rpc)("admin_set_service_availability", {
      _service_slug: slug,
      _enabled: enabled,
    });
    if (err) setError(err.message);
      ...
      setAvailability((current) =>
        current.map((row) =>
          row.service_slug === slug
            ? { ...row, is_enabled: enabled, updated_at: new Date().toISOString() }
            : row,
        ),
      );
    setSaving(null);
  ...
  const isEnabled = (slug: string) =>
    availability.find((row) => row.service_slug === slug)?.is_enabled ?? false;
    ...
    <AdminShell title="Services" subtitle="Control what customers can see and purchase">
      ...
      <section className="mb-6 rounded-2xl border bg-card p-4 shadow-card">
        ...
        <p className="mt-1 mb-3 text-xs text-muted-foreground">
          Disabled services are hidden from customers and blocked at transaction creation.
        </p>
        <div className="divide-y rounded-xl border">
          {MANAGED_SERVICES.map(([slug, label]) => {
            const enabled = isEnabled(slug);
            const busy = saving === slug;
            return (
              <div key={slug} className="flex items-center justify-between gap-3 px-3.5 py-3">
                <div>
                  <p className="text-sm font-bold">{label}</p>
                  <p className="text-[11px] text-muted-foreground">
                    {enabled ? "Visible and purchasable" : "Hidden and blocked"}
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  disabled={busy}
                  onClick={() => void toggle(slug, !enabled)}
                  className={`relative h-7 w-12 shrink-0 rounded-full transition-opacity disabled:opacity-50 ${enabled ? "bg-primary" : "bg-muted"}`}
                >
                  <span
      ...
      <p className="mb-4 text-xs text-muted-foreground">
        Performance comes from <code>bill_transactions</code>. Provider and plan data remain dynamic
        from VTpass.
      </p>
      {loading ? (
        <AdminLoading />
