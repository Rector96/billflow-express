...
function AdminTransactions() {
  ...
  const [walletSnap, setWalletSnap] = useState<{ balance_before: number; balance_after: number; amount: number } | null>(null);
        ...
        <>
          ...
          <div className="hidden overflow-x-auto rounded-2xl border bg-card shadow-card lg:block">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="sticky top-0 border-b bg-muted/50 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
                <tr>
                ...
                {rows.map((t) => (
                  ...
                  >
                    <td className="px-3 py-2.5 font-semibold">
                      {t.service}
                      <span className="block text-[11px] font-normal text-muted-foreground">{t.provider}</span>
                    </td>
                    <td className="max-w-[140px] truncate px-3 py-2.5 text-xs text-muted-foreground">{t.user_label}</td>
                    <td className="px-3 py-2.5 font-mono text-[11px]">{t.internal_reference}</td>
                    <td className="px-3 py-2.5 text-[11px] text-muted-foreground">
                      {new Date(t.created_at).toLocaleString("en-NG")}
                    </td>
                    <td className="px-3 py-2.5 text-right font-extrabold tabular-nums">{formatNaira(t.amount, false)}</td>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40" role="dialog">
          ...
          <div className="relative z-10 flex h-full w-full max-w-md flex-col overflow-y-auto border-l bg-background shadow-float">
            ...
            <div className="space-y-4 p-4 text-sm">
              ...
              {careTicket ? (
                <Link to="/admin/care/$ticketId" params={{ ticketId: careTicket.id }} className="block text-center text-xs font-bold text-primary">
                  Open Care {careTicket.ticket_number || ""}
                </Link>
              ) : null}
            </div>
          </div>
...
function CopyRow({ k, v, onCopy }: { k: string; v: string; onCopy: (v: string) => void }) {
  return (
    <div className="flex items-center justify-between gap-3 text-xs">
      ...
      <button
        type="button"
        className="max-w-[60%] truncate text-right font-mono font-semibold text-primary"
        onClick={() => v !== "—" && onCopy(v)}
      >
        {v}
      </button>
