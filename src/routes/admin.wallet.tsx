...
function AdminWallet() {
  ...
  const [rows, setRows] = useState<
    { id: string; reference: string; amount: number; status: TxStatus; type: string; created_at: string }[]
  >([]);
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <KpiCard label="Total liability (balances)" value={formatNaira(n(stats.wallet_balance_total), false)} />
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Manual wallet adjustments are not enabled in the UI. Any future adjustment must use a confirmed,
            audited server RPC (reason + staff id + timestamp).
          </p>
          <div className="mt-5 rounded-2xl border bg-card p-4 shadow-card">
            ...
            {rows.length === 0 ? (
              ...
              <div className="space-y-2">
                {rows.map((r) => (
                  <div key={r.id} className="flex justify-between gap-2 rounded-xl border px-3 py-2 text-sm">
                    <div>
