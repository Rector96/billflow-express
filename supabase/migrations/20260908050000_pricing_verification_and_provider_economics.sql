...
BEGIN
  ...
  IF v_bill_id IS NULL THEN RAISE EXCEPTION 'bill_not_found'; END IF;
  IF v_status IS DISTINCT FROM 'successful' THEN RAISE EXCEPTION 'not_successful'; END IF;
  ...
  v_snapshot := CASE
    WHEN jsonb_typeof(v_meta -> 'vtpass_snapshot') = 'object' THEN v_meta -> 'vtpass_snapshot'
    ELSE '{}'::jsonb
  END;
  v_tx := CASE
  ...
  v_rockpay_fee := coalesce(_rockpay_fee, nullif(v_meta ->> 'rockpay_fee', '')::numeric);
  v_rule_id := coalesce(_pricing_rule_id, nullif(v_meta ->> 'pricing_rule_id', '')::uuid);

  v_profit := CASE
    WHEN v_provider_cost IS NOT NULL THEN round(_customer_amount - v_provider_cost, 2)
    ELSE _profit
  END;

  INSERT INTO public.transaction_profits (
    bill_transaction_id, customer_amount, provider_cost, provider_commission,
    rockpay_fee, profit, pricing_rule_id
  ) VALUES (
    v_bill_id, round(_customer_amount, 2), v_provider_cost, v_provider_commission,
    v_rockpay_fee, v_profit, v_rule_id
  )
