-- Provider settlement and profit recording must only be callable by server code.
-- The trusted wrappers impersonate the authenticated customer only for the
-- ownership check inside the existing settlement functions.

REVOKE EXECUTE ON FUNCTION public.complete_airtime_purchase(text, public.tx_status, text, jsonb)
  FROM authenticated;
REVOKE EXECUTE ON FUNCTION public.complete_bill_purchase(text, public.tx_status, text, jsonb)
  FROM authenticated;
REVOKE EXECUTE ON FUNCTION public.record_transaction_profit(text, numeric, numeric, numeric, numeric, numeric, uuid, text, text, text, numeric)
  FROM authenticated;
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.complete_airtime_purchase(
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.record_transaction_profit(
    ...
    _rockpay_fee, _profit, _pricing_rule_id, _service, _provider, _product_code,
    _provider_amount
  );
END;
$$;
