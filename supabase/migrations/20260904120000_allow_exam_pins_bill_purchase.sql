-- Allow exam-pins through start_bill_purchase (WAEC/JAMB via VTpass).
-- Keep education/internet/water/insurance blocked.
...
BEGIN
  ...
  INSERT INTO public.bill_transactions (
    ...
    coalesce(_metadata, '{}'::jsonb) || jsonb_build_object(
      'service_slug', slug,
      'channel', 'vtpass',
      'title', coalesce(_metadata->>'title', 'Exam Pins')
    ),
    rid,
    'vtpass'
  )
  ...
  INSERT INTO public.wallet_transactions (
    ...
    w.id, uid, 'debit', amt, before, after,
    ref, 'completed',
    coalesce(_metadata->>'title', 'Exam Pins'),
    jsonb_build_object('bill_reference', ref, 'service_slug', slug)
  );
...
-- Allow exam-pins inserts via start_bill_purchase; keep education/internet/water/insurance blocked
...
BEGIN
  IF slug IN ('education', 'internet', 'water', 'insurance')
     OR service_name IN ('education', 'internet', 'water', 'insurance') THEN
    RAISE EXCEPTION 'unsupported_service';
  END IF;
  RETURN NEW;
END;
