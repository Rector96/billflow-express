-- Harden bill settlement for Data + reliable VTpass request IDs.
-- Safe to re-run. Does not change balances by itself.

-- 1) VTpass request_id: first 12 chars = YYYYMMDDHHMM in Africa/Lagos
CREATE OR REPLACE FUNCTION public.vtpass_request_id()
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
  lagos_local timestamp;
  prefix text;
BEGIN
  -- Convert UTC now → Lagos wall-clock, then format (VTpass requirement)
  lagos_local := timezone('Africa/Lagos', now());
  prefix := to_char(lagos_local, 'YYYYMMDDHH24MI');
...
-- 2) Ensure Data is an allowed service_slug (in case data migration was skipped)
...
BEGIN
  ...
  IF slug NOT IN ('cable', 'electricity', 'data') THEN
