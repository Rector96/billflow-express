-- ============================================================================
-- RockPay: Supabase Production SQL Setup for Multi-Vendor Bill Purchases & Exam PINs
-- ============================================================================
-- Copy and paste this script directly into your Supabase Dashboard:
-- SQL Editor -> New Query -> Paste -> Run
--
-- What this enables:
-- 1. Updates start_bill_purchase to support 'exam-pins', 'education', 'airtime', 'data', 'cable', 'electricity'
-- 2. Quantity & Unit Amount handling for Exam PINs (WAEC, NECO, NABTEB, JAMB)
-- 3. Updates complete_bill_purchase to support both authenticated clients and server-side service_role
-- 4. Creates trusted_complete_bill_purchase for automatic webhook and backend multi-vendor settlements
-- 5. Adds high-speed indexes on bill_transactions and wallet_transactions
-- ============================================================================

-- 1. Function: start_bill_purchase (validates PIN, debits wallet atomically, generates provider reference)
...
-- 2. Function: complete_bill_purchase (finalizes transaction, issues PIN/token, handles automatic refund if failed)
...
BEGIN
  ...
  -- Failed outcome -> atomic wallet refund
...
-- 3. Function: trusted_complete_bill_purchase (for server functions and webhooks)
